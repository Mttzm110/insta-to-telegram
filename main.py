...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "سلام! لینک اینستاگرام بفرست تا عکس یا ویدیو رو برات بفرستم."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "instagram.com" not in url:
        await update.message.reply_text("لطفا فقط لینک‌های معتبر اینستاگرام ارسال کن.")
        return
    ...
    try:
        ...
        await msg.edit_text(
            "❌ خطایی رخ داد. لطفا لینک را چک کن و دوباره تلاش کن."
        )
...
# ایجاد event loop اختصاصی برای webhook
...
# مسیر ping برای سرویس‌های uptime
@app.route("/ping", methods=["GET"])
def ping():
    return Response("pong", status=200)

# مسیر روت برای اطمینان از اجرای سرور
